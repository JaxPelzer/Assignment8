...
app.get('/sortCourses', async (req, res) => {
    try {
        let courses = await Course.find({}).lean();
        courses.sort((a, b) => {
...
app.post('/editStudentById', async (req, res) => {
    try {
        let student = await Student.find({ _id: req.body.id })
...
app.post('/deleteCourseById', async (req, res) => {
    try {
        ...
        if (result) {
            ...
            console.log('(message: Course deleted)');
...
app.post('/editCourseByID', async (req, res) => {
    try {
        let course = await Course.find({ _id: req.body.id })
